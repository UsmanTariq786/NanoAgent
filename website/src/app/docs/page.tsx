          ...
          <article className="min-w-0 max-w-[840px]">
            ...
            <DocSection id="install" heading="Install">
              ...
              <DocTable
                ...
                rows={[
                  ["Windows x64", <a key="w" href="https://github.com/getnanoai/NanoAgent/releases/latest/download/NanoAgent.Desktop-win-x64-setup.exe" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">Installer</a>],
                  ["Linux x64", <a key="lx" href="https://github.com/getnanoai/NanoAgent/releases/latest/download/NanoAgent.Desktop-linux-x64.zip" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">Zip</a>],
                  ["Linux arm64", <a key="la" href="https://github.com/getnanoai/NanoAgent/releases/latest/download/NanoAgent.Desktop-linux-arm64.zip" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">Zip</a>],
                  ["macOS x64", <a key="mx" href="https://github.com/getnanoai/NanoAgent/releases/latest/download/NanoAgent.Desktop-osx-x64.zip" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">Zip</a>],
                  ["macOS arm64", <a key="ma" href="https://github.com/getnanoai/NanoAgent/releases/latest/download/NanoAgent.Desktop-osx-arm64.zip" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">Zip</a>],
                ]}
              />
              ...
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>https://github.com/getnanoai/NanoAgent/releases/latest</code>
              </pre>

              <h3>CLI</h3>
              <h4>Curl</h4>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>curl -fsSL https://raw.githubusercontent.com/getnanoai/NanoAgent/master/scripts/install.sh | bash</code>
              </pre>
              <h4>Windows PowerShell</h4>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>irm https://raw.githubusercontent.com/getnanoai/NanoAgent/master/scripts/install.ps1 | iex</code>
              </pre>
              <h4>NPM</h4>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
...
dotnet pack NanoAgent/NanoAgent.csproj -c Release`}</code>
            ...
            <DocSection id="license" heading="License">
              ...
              <DocNote>
                ...
                <a href="https://github.com/getnanoai/NanoAgent" target="_blank" rel="noopener noreferrer" className="text-[var(--color-acc-1)] hover:underline">
            ...
            {/* Gateway Overview */}
            <DocSection id="gateway-overview" heading="Gateway Setup &amp; Connection">
              <p>
                <strong>NanoAgent Gateway</strong> is the central control plane for engineering teams. It provides a single endpoint (<code>https://app.getnanoai.com/v1</code>) to track model spend, set team budgets, and audit AI usage.
              </p>
              
              <h3>How to connect NanoAgent (Zero Code)</h3>
              <p>You don't need to write any code to use the Gateway with NanoAgent. Simply configure it in your terminal or IDE:</p>
              
              <h4>Method 1: Interactive Setup</h4>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>nanoai setup</code>
              </pre>
              <p className="text-sm text-[var(--color-text-mut)] ml-4">Select <strong>Gateway</strong> from the provider list and paste your Gateway API key.</p>

              <h4>Method 2: Environment Variable</h4>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>export NANOAGENT_API_KEY="your_gateway_key_here"</code>
              </pre>
            ...
            {/* Gateway Integration */}
            <DocSection id="gateway-integration" heading="Connecting Custom Apps &amp; Scripts (Optional)">
              <p>
                If your team has existing Python or Node.js scripts using OpenAI, you can point them to the Gateway by setting the standard <code>OPENAI_BASE_URL</code>:
              </p>
              <pre className="relative m-4 p-[30px_18px_18px] overflow-x-auto bg-[#050505] border border-[var(--color-border)] rounded-xl font-[var(--font-mono)] text-[13px] leading-[1.7] text-[var(--color-text)]">
                <code>{`export OPENAI_BASE_URL="https://app.getnanoai.com/v1"
export OPENAI_API_KEY="your_gateway_key_here"`}</code>
              </pre>
              <p className="text-sm text-[var(--color-text-mut)] ml-4">
            ...
            <DocSection id="gateway-policy" heading="Gateway Dashboard &amp; Spend Controls">
              <p>
                Team leads and administrators manage the Gateway via the web dashboard at <a href="https://app.getnanoai.com" target="_blank" rel="noopener noreferrer" className="text-[#f4c489] hover:underline font-semibold">app.getnanoai.com</a>:
              </p>
              <ul className="text-[var(--color-text-mut)] text-[15px] pl-[22px] m-3">
                <li className="m-[6px_0]"><strong>Spend Attribution:</strong> See exactly how much each team member or project spends on AI models in real time.</li>
                <li className="m-[6px_0]"><strong>Monthly Budget Caps:</strong> Set dollar limits per developer to prevent accidental overuse.</li>
                <li className="m-[6px_0]"><strong>Model Access Rules:</strong> Choose which models (Claude 3.7, GPT-4o, DeepSeek, Local Ollama) developers can use.</li>
                <li className="m-[6px_0]"><strong>Security Audit Logs:</strong> Review timestamped logs of all API activity for compliance.</li>
              </ul>
            </DocSection>

            {/* NanoForge Overview */}
            <DocSection id="forge-overview" heading="NanoForge App Builder">
              <p>
                <strong>NanoForge</strong> turns natural language prompts into full-stack web applications (React frontend + Express API + PostgreSQL database).
              </p>
              
              <h3>How to build an app</h3>
              <ol className="text-[var(--color-text-mut)] text-[15px] pl-[22px] m-3">
                <li className="m-[6px_0]">Go to <a href="/nanoforge" className="text-[#b07cff] hover:underline font-semibold">NanoForge</a> and type what you want to build (e.g. <em>"Project dashboard with tasks, auth, and dark mode"</em>).</li>
                <li className="m-[6px_0]">Click <strong>Build</strong>. NanoForge generates the code, sets up the database schema, and spins up a live container.</li>
                <li className="m-[6px_0]">Interact with your live running app in the preview browser tab.</li>
              </ol>
            </DocSection>

            {/* NanoForge Containers */}
            <DocSection id="forge-containers" heading="Extending NanoForge Apps Locally">
              <p>
                Every NanoForge app is backed by a standard Git repository:
              </p>
              <ul className="text-[var(--color-text-mut)] text-[15px] pl-[22px] m-3">
                <li className="m-[6px_0]"><strong>Inspect Git Diffs:</strong> Review every commit generated by your prompts on GitHub before merging.</li>
                <li className="m-[6px_0]"><strong>Open in NanoAgent:</strong> Clone the repository to your machine and run <code className="code-inline">nanoai</code> to continue coding, testing, and adding features locally.</li>
              </ul>
            </DocSection>
