...
public sealed class VoiceDictationService : IVoiceDictationService
{
    ...
    {
        string? configured = Environment.GetEnvironmentVariable("NANOAGENT_VOICE_RUNTIME");
        if (!string.IsNullOrWhiteSpace(configured))
        {
            return configured.Trim();
        }

        string fileName = OperatingSystem.IsWindows() ? "nanoagent-voice.exe" : "nanoagent-voice";
    ...
    private static string GetDefaultSettingsPath()
    {
        ...
        return Path.Combine(baseDirectory, "NanoAgent", "voice.json");
    }
