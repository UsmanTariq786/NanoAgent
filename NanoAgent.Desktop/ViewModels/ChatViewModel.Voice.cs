using NanoAgent.Application.Voice;
...
public partial class ChatViewModel
{
    ...
    public async Task<bool> TryHandleVoiceCommandAsync()
    {
        ...
        if (command.Equals("/voice setup", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Messages.Add(new ChatMessage("NanoAgent", "Voice setup saved."));
            }
            return true;
        }
        ...
        Messages.Add(new ChatMessage("NanoAgent", "Usage: /voice, /voice setup, or /voice update"));
        return true;
    }
    ...
    public async Task DictateVoiceAsync()
    {
        ...
        try
        {
            ...
            Messages.Add(new ChatMessage("NanoAgent", "Voice dictation added to input."));
        }
        catch (Exception exception)
        {
            Messages.Add(new ChatMessage("NanoAgent", $"Voice error: {exception.Message}"));
        }
        finally
        {
    ...
    public async Task UpdateVoiceModelsAsync()
    {
        ...
        try
        {
            ...
            Messages.Add(new ChatMessage("NanoAgent", "Voice models are up to date."));
        }
        catch (Exception exception)
        {
            Messages.Add(new ChatMessage("NanoAgent", $"Voice update error: {exception.Message}"));
        }
        finally
        {
            IsVoiceDictating = false;
    ...
    private async Task<VoiceSettings?> ConfigureVoiceAsync()
    {
        ...
        if (models.Count == 0)
        {
            Messages.Add(new ChatMessage("NanoAgent", "No voice models are available."));
            return null;
        }
